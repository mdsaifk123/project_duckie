...
class Odometry_lab2:
    ...
    def __init__(self):
        ...
        rospy.Subscriber(self.input_topic, DistWheel, self.callback)
        # set the last measured timestamp to the initialization time
        self.last_measure_time = time.time()

        self.pose_pub = rospy.Publisher(self.output_topic, Pose2D, queue_size=10)

    def update_pose_from_dist_wheel(self, dist_wheel):
        ...
        delta_s = self.calculate_change_in_arc_from_wheel_dist(dist_wheel)
        delta_theta = self.calculate_change_in_angle_from_wheel_dist(dist_wheel)
    ...
    def calculate_change_in_arc_from_wheel_dist(self, dist_wheel):
        # extract the left and right wheel distance data
        l_dist = dist_wheel.dist_wheel_left 
        r_dist = dist_wheel.dist_wheel_right
        delta_s = (l_dist + r_dist) / 2
        return  delta_s

    def calculate_change_in_angle_from_wheel_dist(self, dist_wheel):
        # extract the left and right wheel distance data
        l_dist = dist_wheel.dist_wheel_left 
        r_dist = dist_wheel.dist_wheel_right
    ...
    def callback(self, msg):
        ...
        self.update_pose_from_dist_wheel(dist_wheel)
